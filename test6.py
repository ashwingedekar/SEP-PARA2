        ...
        try:
            ...
            if not selected_data.empty:
                if flags.get("max") == '1':
                    ...
                    # Check if the ID is 9619 and limits are available
                    if id_value == '9619' and upper_warning_limit is not None and upper_error_limit is not None:
                        ...
                        if max_speed_value > upper_error_limit and max_speed_value <= upper_warning_limit:
                            output_text += f" ALERT !!! MAX SPEED crosses Upper Error Limit but is within Upper Warning Limit ({upper_error_limit:.2f} Mbps and {upper_warning_limit:.2f} Mbps)\n\n"
                        elif max_speed_value <= upper_error_limit and max_speed_value > upper_warning_limit:
                            output_text += f" ALERT !!! MAX SPEED Crosses Upper Warning Limit but is within Upper Error Limit ({upper_warning_limit:.2f} Mbps and {upper_error_limit:.2f} Mbps)\n\n"
                        elif max_speed_value <= upper_error_limit and max_speed_value <= upper_warning_limit:
                            output_text += f" MAX SPEED is within both Upper Error Limit and Upper Warning Limit ({upper_error_limit:.2f} Mbps and {upper_warning_limit:.2f} Mbps)\n\n"
                        else:
                            output_text += f" MAX SPEED crosses both Upper Error Limit and Upper Warning Limit ({upper_error_limit:.2f} Mbps and {upper_warning_limit:.2f} Mbps)\n\n"
